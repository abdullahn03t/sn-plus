...
export default function CompanyManager({ companies }) {
  ...
  function startEdit(company) {
    ...
    setEditAr(company.name_ar);
    setEditEn(company.name_en);
  }

  async function saveEdit(id) {
    const supabase = createSupabaseBrowserClient();
    await supabase.from('companies').update({ name_ar: editAr, name_en: editEn }).eq('id', id);
    setEditingId(null);
    router.refresh();
  }
    ...
    <div>
      ...
      <div className="mt-6 space-y-3">
        {companies.map((company) => (
          <div key={company.id} className="flex items-center justify-between rounded-xl bg-white border border-sage-line p-4">
            {editingId === company.id ? (
              ...
              <div>
                <p className="font-semibold text-ink">{company.name_ar}</p>
                <p className="text-sm text-ink/50">{company.name_en}</p>
              </div>
            )}
            ...
            <div className="flex items-center gap-3 ms-4">
              {editingId === company.id ? (
                <>
                  <button onClick={() => saveEdit(company.id)} className="text-pine"><Check size={18} /></button>
                  <button onClick={() => setEditingId(null)} className="text-ink/40"><X size={18} /></button>
                </>
              ) : (
                <>
                  <button onClick={() => startEdit(company)} className="text-pine"><Pencil size={16} /></button>
                  <button onClick={() => handleDelete(company.id, t('confirmDeleteCompany'))} className="text-red-600"><Trash2 size={16} /></button>
                </>
              )}
            </div>
          </div>
        ))}
        {companies.length === 0 && <p className="text-ink/50">{t('noCompanies')}</p>}
      </div>
    </div>
  );
}